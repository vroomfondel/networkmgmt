.PHONY: tests help install venv lint isort tcheck build commit-checks prepare gitleaks pypibuild pypipush docker update-all-dockerhub-readmes
SHELL := /usr/bin/bash
.ONESHELL:
...
dist/networkmgmt-$(VERSION).tar.gz dist/networkmgmt-$(VERSION)-py3-none-any.whl dist/.touchfile: $(PKG_SOURCES) $(VENV_DEPS) pyproject.toml
...
pypibuild: venv dist/networkmgmt-$(VERSION).tar.gz dist/networkmgmt-$(VERSION)-py3-none-any.whl

dist/.touchfile_push: dist/networkmgmt-$(VERSION).tar.gz dist/networkmgmt-$(VERSION)-py3-none-any.whl
	@$(venv_activated)
